...
{	
  ...
  const mmVector3 u = vertices[0] - vertices[1];
  const mmVector3 v = vertices[2] - vertices[1];
  
  // cross product
  //  mmVector3 normal = Normalize(CrossProd(v, u));
  mmVector3 normal = CrossProd(v, u);
  
  // ray direction vector
  const mmVector3 dir = ray.direction;
  const mmVector3 w0 = ray.origin - vertices[1];
  ...
  const mmVector3 pt = ray.origin + t * dir;
  const mmVector3 w = pt - vertices[1];
